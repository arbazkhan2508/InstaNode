...
const { uploadposts, uploadprofile, uploadstory } = require("../model/grid");
...
router.get("/auth/facebook/callback",
  ...
  function (req, res) {
    var data = res.req.user.name;
    res.redirect('/signf/'+data);
  }
);
...
router.get('/signf/:name',function(req,res){
  var data = req.params.name;
   res.render('signf',{data:data});
})

// update user
router.post("/fbupdate", isLoggedIn, function (req, res) {
  // console.log(req.body.name,"dhgdhjdhjdgdgjdgj");
...
router.get("/archive",async function (req, res) {
  ...
  res.render("Storyarchive",{user:user});
});

router.get("/intrect",async function (req, res) {
  let user = await userModel.findOne({ username: req.user.username })
  .populate({
    path: "likes",
    populate: {
  ...
  res.render("intrec",{user:user});
});

router.get("/cmntint",async function(req, res){
  let user = await userModel.findOne({username:req.user.username});
  let allp = await postModel.find().populate('owner')
  .populate({
    path: "comment",
    populate: {
      path: "owner",
  ...
  res.render('Ucmntint',{allp:allp,user:user});
  
})

router.get("/P&V",async function (req, res) {
  let user = await userModel.findOne({ username: req.user.username })
   .populate('Uposts')
  res.render("P&V",{user:user});
});

router.get("/P&V/Reel",async function (req, res) {
  let user = await userModel.findOne({ username: req.user.username })
  .populate('reels')
  res.render("P&VPost",{user:user});
});


router.post("/userprofileimg",
...
router.post("/uploadpost",uploadposts.single("posts"),
  async function (req, res) {
    ...
    if (extension === "mp4") {
      ...
      user.posts.push(userPosts);
      user.Uposts.push(userPosts);
    }
    await user.save();
    // console.log(filename,".nmssdbmbdmnfbdmfbdmfbdnmbsmndbsmbmbmfbfmsnfmnfbmnfbsfmnbfmnbmn");
    res.redirect('/uplpost/'+userPosts.post);
  }
);

router.get('/uplpost/:data',async function(req,res){
  let user = await userModel.findOne({username:req.user.username})
  var data = req.params.data;
  console.log(data,"djhjdjkd");
   res.render('uplpost',{data:data,user:user});
})

// update uploaded post

router.post('/uplpost',function(req,res){
  ...
  .then(function (post) {
    console.log(post,"dbjcbjbbcbbcjcbhjbbchjbccbhjcbcbccccbcbcbbcbcbcbcbmncbmncbn");
    res.redirect("/profile");
  });
})


router.post("/uploadstory",uploadstory.single("filename"),
  async function (req, res) {
    let user = await userModel.findOne({ username: req.user.username });
    var uri = req.file.filename;
    var extension = uri.substring(uri.lastIndexOf(".") + 1);
...
router.get("/delete/:id", async function (req, res) {
  ...
  let index1 = user.Uposts.indexOf(req.params.id);
  user.posts.splice(index, 1);
  user.Uposts.splice(index, 1);
...
router.get('/home', async (req, res) => {
  ...
  try {
    ...
    res.render('home', { posts, totalPages, currentPage: page });
  } catch (error) {
    console.error(error);
...
//   const limit = 10; // Number of posts to fetch on each request
...
//     res.render('home', { posts, currentPage,user }); // Pass current page to template
//   } catch (error) {
//     res.status(500).send('An error occurred.');
//   }
// });
...
//     let posts = await postModel.find().limit(1).skip(skip).populate("owner");
//     console.log(posts,"its posts");
//     res.render("home", {
//       posts: posts,
//       user: user,
...
router.post('/loadmore', (req, res) => {
  const receivedData = req.body; // Data sent from the client

  // Process the data, perform MongoDB operations, etc.

  const responseData = { message: 'Data received and processed successfully.' };
...
router.get("/img/:id", isLoggedIn, async function (req, res) {
  ...
  let user = await userModel
    ...
    // console.log(postone.comment,"sdjdbdkjshkjs");  
  let commentOne = await commentModel
    .findOne({ _id: postone.comment })
    .populate("likes")
    .populate("owner");
    console.log(commentOne,"jdsshjkjsdkj");
...
router.get("/searchuser/:id", isLoggedIn, async function (req, res) {
  ...
  let postuser = await userModel
    .findOne({ _id: postone.owner })
    .populate({
      path: "posts",
      populate: {
        path: "owner",
  ...
  let commentOne = await commentModel
    ...
    .populate("likes")
    .populate("owner");
  res.render("userimg", { postone, user,postuser, commentOne, url });
});
// searchuserPosts
